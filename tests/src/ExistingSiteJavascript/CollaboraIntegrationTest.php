...
class CollaboraIntegrationTest extends ExistingSiteSelenium2DriverTestBase {
    ...
    public function testCollaboraPreview(): void {
        // @todo Make sure there is always a document media type.
        $user = $this->createUser([
            'preview document in collabora',
        ]);
        $this->drupalLogin($user);
        $text_file_basename = 'shopping-list';
        $text_file_content = 'Chocolate, pickles';
        $media = $this->createDocumentMedia('Shopping list', $text_file_basename, $text_file_content);
        ...
        $this->assertSame(
            sprintf(
                ...
                str_word_count($text_file_content),
                mb_strlen($text_file_content),
            ),
